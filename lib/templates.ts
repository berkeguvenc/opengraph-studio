...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  return {
    ...
    openGraph: {
      ...
      alternateLocale: [${state.secondaryLocales.split(',').map(l => `'${escapeStr(l.trim())}'`).join(', ')}],
      type: 'website',
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'flex-start' }}>
          ${state.tags.length > 0 ? `<div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
            ${state.tags.map(tag => `<div style={{ display: 'flex', padding: '8px 16px', borderRadius: '9999px', backgroundColor: '${state.accentColor}30', color: '${state.accentColor}', fontSize: '20px', fontWeight: 600, letterSpacing: '-0.01em' }}>${escapeStr(tag)}</div>`).join('\n            ')}
          </div>` : ''}
...
`
  ...
  return [
    {
      tabName: 'opengraph-image.tsx',
      filename: state.i18nEnabled ? 'app/[locale]/opengraph-image.tsx' : 'app/opengraph-image.tsx',
      language: 'tsx',
      content: (i18nCode + componentCode).trim()
    },
    {
      tabName: 'generateMetadata',
      filename: state.i18nEnabled ? 'app/[locale]/layout.tsx' : 'app/layout.tsx',
      language: 'tsx',
      content: metaCode.trim()
    }
  ]
}
...
useSeoMeta({
  ...
  ogTitle: '${eTitle}',
  ogDescription: '${eDesc}',
  ogImage: 'https://yourdomain.com/api/og', // Absolute URL required for social platforms
  ogSiteName: '${eBrand}',
...
// })
  ...
  const ogContent = `import { defineEventHandler } from 'h3'
...
export default defineEventHandler((event) => {
  ...
  return { message: "Generated Image Here" }
})`
...
// routes/web.php
// Route::get('/api/og', [OgImageController::class, 'generate']);`
  ...
  return [
    ...
    {
      tabName: 'Controller.php',
      filename: 'app/Http/Controllers/OgImageController.php',
      language: 'php',
      content: phpContent
    }
  ]
